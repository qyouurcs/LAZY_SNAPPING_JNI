		...
		for( int n = 0; n < numk; n++ )
		{
			...
			for( int y = y1; y < y2; y++ )
			{
				for( int x = x1; x < x2; x++ )
				{
					...
					dist =			(1-depthFactor)*(l - kseedsl[n])*(l - kseedsl[n]) +
									(1-depthFactor)*(a - kseedsa[n])*(a - kseedsa[n]) +
									(1-depthFactor)*(b - kseedsb[n])*(b - kseedsb[n]) ;
	...
	const int					isLAB)
{
    ...
    //--------------------------------------------------
    if(isLAB)//LAB, the default option
    {
	...
	PerformSuperpixelSLIC(kseedsl, kseedsa, kseedsb, kseedsx, kseedsy, klabels, STEP, edgemag,depthFactor,compactness);
...
void SLIC::DoSuperpixelSegmentation_ForGivenNumberOfSuperpixels(
	...
	const double&				depthFactor,
    const double&                                   compactness,
	const int					isLAB)//weight given to spatial distance
{
    const int superpixelsize = 0.5+double(width*height)/double(K);
    DoSuperpixelSegmentation_ForGivenSuperpixelSize(ubuff,width,height,klabels,numlabels,superpixelsize,depthFactor,compactness,isLAB);
}
